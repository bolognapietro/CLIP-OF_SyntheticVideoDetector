...
# CATEGORY_TYPE = 'ANIMALS'  # PEOPLE, OBJECTS, VEICHLES, ANIMALS, LANDSCAPES
...
def plot_graph(data, ranges, x_ticks, x_positions, graph_name, threshold=0.5):
    ...
    plt.savefig(os.path.join(os.path.dirname(__file__), f'results/{graph_name}.png'))  # Save the figure
    plt.show()

if __name__ == '__main__':
    CATEGORY_TYPE = input("Enter the category type (PEOPLE, OBJECTS, VEICHLES, ANIMALS, LANDSCAPES): ")
    ...
    people = ['Soldier', 'Skier', 'Dancer', 'Farmer', 'Pilot', 'TaxiDriver', 'Chef', 'Doctor']
    objects = ['Ball', 'Purse', 'TV', 'Sofa', 'Drone']
    veichles = ['Car', 'Bike', 'RoadFullOfCars', 'Tractor', 'Airplane', 'Rocket']
    animals = ['Horse', 'Cow', 'Butterfly', 'Mouse', 'Croccodile', 'Dolphin', 'PolarBear', 'Tiger']
    landscapes = ['Thunder', 'River', 'Shore']

    filtered_values = []
    grouped_labels_filtered = []
    for i, (name, value) in enumerate(zip(data_final["filename"], data_final["prediction"])):
        if name.split('_')[0] in people and CATEGORY_TYPE == 'PEOPLE':
            filtered_values.append({'filename': name, 'prediction': value})
            grouped_labels_filtered.append(grouped_labels[i])
        elif name.split('_')[0] in objects and CATEGORY_TYPE == 'OBJECTS':
            filtered_values.append({'filename': name, 'prediction': value})
            grouped_labels_filtered.append(grouped_labels[i])
        elif name.split('_')[0] in veichles and CATEGORY_TYPE == 'VEICHLES':
            filtered_values.append({'filename': name, 'prediction': value})
            grouped_labels_filtered.append(grouped_labels[i])
        elif name.split('_')[0] in animals and CATEGORY_TYPE == 'ANIMALS':
            filtered_values.append({'filename': name, 'prediction': value})
            grouped_labels_filtered.append(grouped_labels[i])
        elif name.split('_')[0] in landscapes and CATEGORY_TYPE == 'LANDSCAPES':
            filtered_values.append({'filename': name, 'prediction': value})
            grouped_labels_filtered.append(grouped_labels[i])

    data_filtered = pd.DataFrame(filtered_values)  

    # Calculate accuracy for each group
    threshold = 0.45
    accuracy = compute_accuracy(data_filtered, grouped_labels_filtered, threshold)

    # Set plot parameters
    ranges, x_ticks, x_positions = set_plot_params(grouped_labels_filtered, accuracy)

    # Plot the graph
    plot_graph(data_filtered, ranges, x_ticks, x_positions, f'CLIP-OF results category {CATEGORY_TYPE}', threshold)